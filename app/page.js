...
export default async function Home() {
  ...
  return (
    <div className="p-3 md:p-4 flex  flex-col gap-24 ">
      ...
      <section>
        <h1 className=" font-mono mb-6  text-xl ">FEATURED / <a href='/work'> <div className='inline-flex gap-2 justify-baseline group'><div>WORK</div> <ArrowRight className="transition-transform duration-200 group-hover:translate-x-2.5"/></div></a> </h1>
        <div className="grid grid-cols-1  md:grid-cols-2 gap-4 ">
          {featuredWork.length > 0 ? (
            featuredWork.map((item) => (
              <Link key={item.id} href={`/work/${encodeURIComponent(item.slug)}`}>
       ...
       <section>
        <h1 className=" font-mono mb-6  text-xl ">FEATURED / <a href='/writing'> <div className='inline-flex gap-2 justify-baseline group'><div>WRITING</div> <ArrowRight className="transition-transform duration-200 group-hover:translate-x-2.5"/></div></a> </h1>
        <div className="grid grid-cols-1  md:grid-cols-2 gap-4 ">
          {featuredWriting.length > 0 ? (
            featuredWriting.map((item) => (
              <Link key={item.id} href={`/writing/${encodeURIComponent(item.slug)}`}>
                <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-6 h-[200px] flex flex-col justify-between hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                  <div>
                    <h3 className="font-semibold text-lg mb-2">{item.frontmatter.title}</h3>
                    <p className="text-gray-600 dark:text-gray-300 text-sm">{item.frontmatter.description}</p>
